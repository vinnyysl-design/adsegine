...
def _reorder_roas_acos(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = _keep_first_by_prefix(df, prefixes_norm=("roas_objetivo", "roas_objetivo_n"))

    # detectar a coluna de ROAS objetivo que sobrou
    roas_obj_cols = [c for c in df.columns if _norm_col(c) in ['roas_objetivo', 'roas_objetivo_n']]
...
def _roas_col_name_from_acos_col(col_name: str) -> str:
    ...
    if lc.endswith("_n") or "objetivo_n" in lc or "objetivo n" in lc:
        return "ROAS_Objetivo_N"
    return "ROAS_Objetivo"
...
def format_table_br(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in df_fmt.columns:
        ...
        if _is_money_col(col):
            ...
            df_fmt[col] = serie_num.map(lambda x: f"{x:.2f}x".replace(".", ",") if pd.notna(x) else "")
...
def main():
    ...
    if not plan15.empty:
        ...
        # Usamos .map() em vez de .applymap() para compatibilidade com Pandas 2.1.0+
        st.dataframe(
            plan15.style.map(color_fase, subset=["Fase"]),
            use_container_width=True,
            hide_index=True
        )
    else:
