...
def _write_sheet_with_formatting(writer: pd.ExcelWriter, df: pd.DataFrame, sheet_name: str, formats: Dict[str, xlsxwriter.format.Format]):
    ...
    for col_num, col_name in enumerate(df.columns):
        # Largura da coluna
        # Convertemos para string e tratamos valores nulos para evitar erros de tipo
        max_val_len = df[col_name].astype(str).fillna('').map(len).max()
        # Se a coluna estiver vazia, max() pode retornar NaN ou erro dependendo da versão do pandas
